...
async def vk_listener_async():
    ...
    while True:
        try:
            ...
            print("✅ VK API initialized", flush=True)

            # ПОКАЗАТЬ ВСЕ ДОСТУПНЫЕ БЕСЕДЫ
            try:
                dialogs = vk_api_instance.messages.getConversations(count=200)
                print("=" * 50, flush=True)
                print("СПИСОК ДОСТУПНЫХ БЕСЕД", flush=True)
                print("=" * 50, flush=True)
                for item in dialogs["items"]:
                    peer = item["conversation"]["peer"]
                    if peer["type"] == "chat":
                        print(f"PEER={peer['id']}", flush=True)
            except Exception as e:
                print(f"ОШИБКА ПОЛУЧЕНИЯ БЕСЕД: {e}", flush=True)
...
async def main():
    ...
    print("=" * 50, flush=True)
    print("🔧 ВЕРСИЯ КОДА: 5.2.1 (ИСПРАВЛЕН _publish_to_vk)", flush=True)
    print("=" * 50, flush=True)
