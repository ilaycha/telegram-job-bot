...
def build_resume_text(data: dict) -> str:
    ...
    if data.get("age"):
        lines.append(f"🎂 {data['age']}")
    if data.get("position"):
        lines.append(f"💼 Желаемая должность: {data['position']}")
    if data.get("experience"):
        lines.append(f"📅 Опыт работы:\n{data['experience']}")
    if data.get("skills"):
        lines.append(f"🛠 Навыки:\n{data['skills']}")
    if data.get("education"):
        lines.append(f"🎓 Образование:\n{data['education']}")
    if data.get("contact"):
        lines.append(f"📞 Контакты: {data['contact']}")
    return "\n\n".join(lines)
...
async def moderation_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        try:
            ...
            await context.bot.send_message(user_id, "✅ Ваша публикация одобрена и опубликована.")
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            V_SALARY:      [
                MessageHandler(filters.TEXT & ~filters.COMMAND, v_salary_step),
                CallbackQueryHandler(v_skip_button, pattern="^v_skip_"),
            ],
            V_SCHEDULE:    [
                MessageHandler(filters.TEXT & ~filters.COMMAND, v_schedule_step),
                CallbackQueryHandler(v_skip_button, pattern="^v_skip_"),
            ],
            V_DESCRIPTION: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, v_description_step),
                CallbackQueryHandler(v_skip_button, pattern="^v_skip_"),
            ],
            V_CONTACT:     [MessageHandler(filters.TEXT & ~filters.COMMAND, v_contact_step)],
            V_PREVIEW:     [
                CallbackQueryHandler(v_preview_buttons, pattern="^v_"),
            ...
            R_AGE:         [
                MessageHandler(filters.TEXT & ~filters.COMMAND, r_age_step),
                CallbackQueryHandler(r_skip_button, pattern="^r_skip_"),
            ],
            R_POSITION:    [
                MessageHandler(filters.TEXT & ~filters.COMMAND, r_position_step),
                CallbackQueryHandler(r_skip_button, pattern="^r_skip_"),
            ],
            R_EXPERIENCE:  [
                MessageHandler(filters.TEXT & ~filters.COMMAND, r_experience_step),
                CallbackQueryHandler(r_skip_button, pattern="^r_skip_"),
            ],
            R_SKILLS:      [
                MessageHandler(filters.TEXT & ~filters.COMMAND, r_skills_step),
                CallbackQueryHandler(r_skip_button, pattern="^r_skip_"),
            ],
            R_EDUCATION:   [
                MessageHandler(filters.TEXT & ~filters.COMMAND, r_education_step),
                CallbackQueryHandler(r_skip_button, pattern="^r_skip_"),
            ],
            R_CONTACT:     [MessageHandler(filters.TEXT & ~filters.COMMAND, r_contact_step)],
            R_PREVIEW:     [
                CallbackQueryHandler(r_preview_buttons, pattern="^r_"),
                CallbackQueryHandler(back_to_main, pattern="^main_menu$"),
