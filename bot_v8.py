...
async def main():
    ...
    # ВАЖНО: Используем add_handlers для правильной регистрации в v22+
    # Порядок важен: специфичные обработчики первыми, общие - последними
    telegram_app.add_handlers([
        # Глобальный /start (работает всегда)
        CommandHandler("start", global_start),
        
        # Модерация и удаление (специфичные паттерны)
        CallbackQueryHandler(moderation_buttons, pattern="^(approve|reject):"),
        CallbackQueryHandler(delete_publication, pattern="^delete:"),
        
        # Команды
        CommandHandler("my", my_publications_command),
        CommandHandler("adminstats", admin_stats),
        CommandHandler("export", admin_export),
    ])
    
    # ConversationHandler добавляем отдельно
    telegram_app.add_handler(conv_handler)
    
    # Выводим все зарегистрированные обработчики для отладки
    print("📋 Зарегистрированные обработчики:", flush=True)
    for handler in telegram_app.handlers:
        handler_type = handler.__class__.__name__
        if hasattr(handler, 'callback'):
            print(f"  - {handler_type}: {handler.callback.__name__}", flush=True)
        else:
            print(f"  - {handler_type}: {handler}", flush=True)
    
    # Бот-чистильщик
    if CLEANER_BOT_TOKEN:
        try:
            cleaner_app = Application.builder().token(CLEANER_BOT_TOKEN).build()
            cleaner_app.add_handlers([
                MessageHandler(
