        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(tries):
                try:
                    ...
                    if attempt == tries - 1:
                        print(f"❌ Исчерпаны попытки для {func.__name__}: {e}")
                        raise
                    print(f"⚠️ Попытка {attempt + 1}/{tries} для {func.__name__}: {e}")
...
async def publish_to_both_platforms(pub_id: str, context) -> bool:
    ...
    if not pub:
        print(f"❌ Публикация {pub_id} не найдена")
        return False
    ...
    try:
        ...
        print(f"✅ Опубликовано в Telegram: {pub_id}")
    except Exception as e:
    ...
    try:
        ...
        if vk_message_id:
            print(f"✅ Опубликовано в VK (чат {vk_chat_id}): {pub_id}")
    except Exception as e:
        print(f"⚠️ Ошибка публикации в VK: {e}")
...
async def delete_from_both_platforms(pub_id: str, context) -> bool:
    ...
    if pub['tg_channel_message_id']:
        try:
            ...
            print(f"🗑 Удалено из Telegram: {pub_id}")
        except Exception as e:
            print(f"⚠️ Не удалось удалить из Telegram: {e}")
    ...
    if pub['vk_message_id'] and pub['vk_chat_id']:
        try:
            if vk_api_instance:
                ...
                print(f"🗑 Удалено из VK (чат {pub['vk_chat_id']}): {pub_id}")
        except Exception as e:
            print(f"⚠️ Не удалось удалить из VK: {e}")
...
async def _skip_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return ConversationHandler.END  # Фактическое состояние обрабатывается в вызывающей функции

# ----- Функции-фабрики для создания обработчиков -----
...
async def _send_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"⚠️ Не удалось отправить на модерацию: {e}")
...
async def moderation_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pub_data = context.bot_data.get(pub_id)
    if not pub_data:
...
async def delete_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"⚠️ Не удалось уведомить модераторов: {e}")
...
async def delete_system_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.new_chat_members or update.message.left_chat_member:
        try:
            ...
            print(f"⚠️ Не удалось удалить системное сообщение: {e}")

# ============================================
# VK: Обработчик сообщений
# ============================================
async def send_vk_message(user_id: str, text: str, keyboard: Optional[Dict] = None):
    ...
    if vk_api_instance:
        try:
            ...
            print(f"❌ Ошибка отправки VK-сообщения: {e}")
...
async def show_vk_main_menu(user_id: str):
    ...
    keyboard = {
        "one_time": False,
        "buttons": [
            [{"action": {"type": "text", "label": "📌 Создать вакансию"}, "color": "primary"}],
            [{"action": {"type": "text", "label": "📄 Создать резюме"}, "color": "primary"}]
        ]
    }
...
async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    ...
    if state == VKState.MAIN_MENU:
        if 'ваканси' in text.lower():
            ...
            await send_vk_message(
                user_id,
                "Пожалуйста, выберите кнопкой: 'Создать вакансию' или 'Создать резюме'"
            )
            return VKState.MAIN_MENU
...
async def vk_listener_async():
    ...
    if not VK_TOKEN or VK_GROUP_ID == 0:
        print("⚠️ VK_TOKEN или VK_GROUP_ID не заданы — VK-часть отключена")
        return
    ...
    try:
        ...
        print("🔵 VK LongPoll запущен с поддержкой состояний...")
    except Exception as e:
    ...
    for event in vk_longpoll.listen():
        try:
            ...
            print(f"⚠️ Ошибка в VK-обработчике: {e}")
...
async def maintenance_scheduler():
    ...
    while True:
        try:
            ...
            print(f"✅ Заархивировано: {archived}, отклонено: {rejected}")
        except Exception as e:
...
async def main():
    ...
    if MODERATION_GROUP_ID == 0:
        print("⚠️ MODERATION_GROUP_ID не задан!")
    ...
    if CLEANER_BOT_TOKEN:
        try:
            ...
            print(f"⚠️ Не удалось запустить бота-чистильщика: {e}")
    ...
    if VK_TOKEN and VK_GROUP_ID != 0:
        ...
        print("⚠️ VK отключен (нет VK_TOKEN или VK_GROUP_ID)")
    ...
    print("=" * 50)
    print("🎉 БОТ ЗАПУЩЕН!")
    print(f"📱 Telegram: активен (модерация в чате {MODERATION_GROUP_ID})")
    print(f"📱 VK: {'активен' if (VK_TOKEN and VK_GROUP_ID != 0) else 'отключён'}")
    print("📦 БД: SQLite")
    print("=" * 50)
