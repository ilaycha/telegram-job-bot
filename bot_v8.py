...
async def moderation_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        if DEBUG_CALLBACKS:
            print(f"⚠️ MODERATION: неизвестное действие '{action}'", flush=True)
...
async def main():
    ...
    # ВАЖНО: Добавляем обработчики по одному через add_handler()
    # Порядок важен: специфичные паттерны первыми!
    
    # 1. Глобальный /start (работает всегда, вне диалога)
    telegram_app.add_handler(CommandHandler("start", global_start))
    
    # 2. Модерация (специфичные паттерны approve: и reject:)
    telegram_app.add_handler(CallbackQueryHandler(moderation_buttons, pattern="^(approve|reject):"))
    
    # 3. Удаление (специфичный паттерн delete:)
    ...
    # 5. ConversationHandler добавляем ПОСЛЕДНИМ
    telegram_app.add_handler(conv_handler)
    
    # Выводим количество зарегистрированных обработчиков для проверки
    print(f"📋 Зарегистрировано обработчиков: {len(telegram_app.handlers)}", flush=True)
    for i, handler in enumerate(telegram_app.handlers):
        handler_type = type(handler).__name__
        if isinstance(handler, ConversationHandler):
            print(f"  [{i}] {handler_type}: name='{handler.name}'", flush=True)
        elif isinstance(handler, CommandHandler):
            print(f"  [{i}] {handler_type}: command='{handler.command}'", flush=True)
        elif isinstance(handler, CallbackQueryHandler):
            print(f"  [{i}] {handler_type}: pattern='{handler.pattern}'", flush=True)
        elif isinstance(handler, MessageHandler):
            print(f"  [{i}] {handler_type}: filters={handler.filters}", flush=True)
        else:
