...
async def show_vk_publications(user_id: str):
    ...
    if not publications:
        await send_vk_message(user_id, "📭 У вас пока нет опубликованных объявлений.", get_vk_main_keyboard())
        return
...
# ============================================
# VK: Машина состояний (с кнопками Пропустить)
# ============================================
async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    ...
    if action == "cancel" or text.lower() in ["отменить", "отмена", "❌ отменить"]:
        clear_vk_state(user_id)
    ...
    if state == VKState.V_COMPANY:
        ...
        await send_vk_message(user_id, "💰 Зарплата?", get_vk_cancel_keyboard())
        return VKState.V_SALARY
    
    if state == VKState.V_SALARY:
        if action == "skip" or text.lower() in ['пропустить', '⏭ пропустить']:
            pass  # оставляем None
        else:
            data['salary'] = text
        save_vk_state(user_id, VKState.V_SCHEDULE, data)
        await send_vk_message(user_id, "🕒 График работы?", get_vk_cancel_keyboard())
    ...
    if state == VKState.V_SCHEDULE:
        ...
        await send_vk_message(user_id, "📋 Описание вакансии?", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_NAME:
        ...
        await send_vk_message(user_id, "🎂 Ваш возраст?", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_AGE:
        ...
        await send_vk_message(user_id, "💼 Желаемая должность?", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_POSITION:
        ...
        await send_vk_message(user_id, "📅 Опыт работы?", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_EXPERIENCE:
        ...
        await send_vk_message(user_id, "🛠 Ключевые навыки?", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_SKILLS:
        ...
        await send_vk_message(user_id, "🎓 Образование?", get_vk_cancel_keyboard())
...
async def maintenance_scheduler():
    ...
    while True:
        try:
            ...
            print(f"✅ Обслуживание: заархивировано {archived}, отклонено {rejected}", flush=True)
        except Exception as e:
