...
async def main():
    ...
    # ВАЖНО: Добавляем обработчики ДО ConversationHandler
    # Они будут иметь приоритет
    
    # Глобальный /start — работает всегда
    telegram_app.add_handler(CommandHandler("start", global_start))
    
    # Модерация и удаление (ДО ConversationHandler!)
    ...
    # ConversationHandler с per_message=False (ВАЖНО!)
    conv_handler = ConversationHandler(
        ...
        per_message=False,  # ВАЖНО: False, потому что используем MessageHandler
        name="main_conversation"
    )
    
    # Добавляем ConversationHandler ПОСЛЕДНИМ
