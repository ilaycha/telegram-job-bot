...
async def send_vk_message(user_id: str, text: str, keyboard: Optional[Dict] = None):
    ...
    if vk_api_instance:
        try:
            ...
            print(f"📤 VK: отправлено сообщение пользователю {user_id}", flush=True)
        except Exception as e:
...
async def show_vk_main_menu(user_id: str):
    ...
    print(f"📤 VK: показано главное меню пользователю {user_id}", flush=True)

async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    """Обработка шагов VK (машина состояний)"""
    print(f"🔍 VK: шаг для {user_id}, состояние: {state}, текст: {text[:50]}", flush=True)
...
async def vk_listener_async():
    ...
    try:
        ...
        print("🔵 VK LongPoll запущен с поддержкой состояний...", flush=True)
    except Exception as e:
    ...
    for event in vk_longpoll.listen():
        try:
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                print(f"📨 VK: новое сообщение от {user_id}: {text[:50]}", flush=True)
