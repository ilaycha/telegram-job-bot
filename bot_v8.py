...
async def vk_listener_async():
    ...
    # Бесконечный цикл восстановления VK-подключения
    while True:
        try:
...
async def maintenance_scheduler():
    ...
    while True:
        try:
            ...
            # 1. Архивируем старые публикации (старше 6 месяцев)
            archived = archive_old_publications()
            
            # 2. Отклоняем просроченные заявки (старше 2 месяцев)
            rejected = auto_reject_stale_publications()
            
            # 3. Очищаем брошенные состояния VK (старше 24 часов)
...
async def main():
    ...
    # Запускаем фоновый планировщик обслуживания
