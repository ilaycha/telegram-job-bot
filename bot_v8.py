...
def get_user_publications(user_id: str, platform: Optional[str] = None, status: Optional[str] = None) -> list:
    ...
    if status:
        ...
        query += " AND status NOT IN ('deleted', 'expired')"
...
def save_vk_state(user_id: str, state: str, data: Optional[Dict] = None):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT OR REPLACE INTO vk_user_states (user_id, state, data)
        VALUES (?, ?, ?)
...
# ============================================
def build_skip_keyboard(next_callback: str, label: str = "⏭ Пропустить") -> InlineKeyboardMarkup:
    """Создать клавиатуру с кнопкой пропуска"""
...
def get_reply_target(update):
    """Получить целевое сообщение для ответа"""
    if update.message:
...
async def publish_to_both_platforms(pub_id: str, context) -> bool:
    ...
    # Публикация в VK (не критично при ошибке)
    try:
...
# ============================================
# Telegram: Универсальные обработчики шагов
# ============================================
async def _generic_form_step(update: Update, context: ContextTypes.DEFAULT_TYPE, 
                              field: str, next_state: TGState, prompt: str, 
                              skip_callback: Optional[str] = None):
    """Универсальный обработчик шага формы"""
    if update.message:
    ...
    reply_markup = build_skip_keyboard(skip_callback) if skip_callback else None
    await get_reply_target(update).reply_text(prompt, reply_markup=reply_markup)
    return next_state

async def _skip_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Универсальный обработчик кнопки пропуска"""
    query = update.callback_query
    await query.answer()
    context.user_data[query.data] = True
    return ConversationHandler.END

def make_message_handler(field: str, next_state: TGState, prompt: str, skip_callback: Optional[str] = None):
    """Создать MessageHandler для шага формы"""
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        return await _generic_form_step(update, context, field, next_state, prompt, skip_callback)
    return MessageHandler(filters.TEXT & ~filters.COMMAND, handler)

def make_skip_handler(next_state: TGState, prompt: str, skip_callback: Optional[str] = None):
    """Создать CallbackQueryHandler для кнопки пропуска"""
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        reply_markup = build_skip_keyboard(skip_callback) if skip_callback else None
...
async def my_publications(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /my — показать мои публикации"""
    ...
    await update.message.reply_text(f"📊 Ваши публикации в Telegram (всего: {total}):")
...
async def my_publications_from_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать публикации из меню"""
    query = update.callback_query
    user_id = str(query.from_user.id)
    publications = get_user_publications(user_id, platform='tg')
    
    if not publications:
        await query.message.reply_text("📭 У вас пока нет опубликованных объявлений.")
        return
    
    total = len(publications)
    await query.message.reply_text(f"📊 Ваши публикации (всего: {total}):")
...
# ============================================
# Telegram: Превью и отправка
# ============================================
async def _show_preview(update, context, next_state: TGState):
    """Показать предпросмотр публикации"""
    ...
    await msg.reply_text(f"📌 Предпросмотр {type_name}:\n\n{text}", reply_markup=keyboard)
    return next_state

async def _send_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправить публикацию на модерацию"""
    query = update.callback_query
    await query.answer()
    ...
    await safe_edit(query, "✅ Отправлено на модерацию.", reply_markup=keyboard)
    context.user_data.clear()

async def _cancel_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отменить создание публикации"""
    query = update.callback_query
    await query.answer()
    context.user_data.clear()
...
async def _restart_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Перезапустить создание публикации"""
    query = update.callback_query
    await query.answer()
    form_type = context.user_data.get("form_type")
    context.user_data["form_data"] = {}
...
async def send_vk_message(user_id: str, text: str, keyboard: Optional[Dict] = None):
    ...
    if not vk_api_instance:
        print(f"❌ VK: vk_api_instance не инициализирован! Невозможно отправить сообщение пользователю {user_id}", flush=True)
        return
    ...
    try:
        ...
        if keyboard:
            ...
            print(f"📤 VK: отправка с клавиатурой (размер: {len(keyboard_json)} байт)", flush=True)
        else:
            print(f"📤 VK: отправка БЕЗ клавиатуры", flush=True)
            ...
            [
                {
                    "action": {
                        ...
                        "payload": "{\"action\": \"my_publications\"}"
                    }
                }
            ]
        ]
...
async def show_vk_publications(user_id: str):
    """Показать публикации пользователя VK"""
    ...
    if not publications:
        await send_vk_message(
            ...
            get_vk_main_keyboard()
        )
        return
    
    total = len(publications)
    await send_vk_message(user_id, f"📊 Ваши публикации (всего: {total}):")
                ...
                [
                    {
                        "action": {
                            "type": "text",
                            "label": f"🗑 Удалить эту публикацию",
                            "payload": json.dumps({"action": "delete", "pub_id": pub['id']})
                        }
                    }
                ]
            ]
    ...
    if total > 10:
        await send_vk_message(user_id, f"🔍 Показаны последние 10 из {total}.")
...
async def show_vk_main_menu(user_id: str):
    ...
    print(f"📤 VK: показываю главное меню пользователю {user_id}", flush=True)
    await send_vk_message(
        user_id,
        "👋 Привет! Я бот для публикации вакансий и резюме.\n\nВыберите действие:",
        get_vk_main_keyboard()
    )
...
async def handle_vk_delete(user_id: str, pub_id: str):
    ...
    if pub['vk_message_id'] and pub['vk_chat_id']:
        try:
            if vk_api_instance:
                ...
                print(f"🗑 Удалено из VK: {pub_id}", flush=True)
        except Exception as e:
            print(f"⚠️ Не удалось удалить из VK: {e}", flush=True)
    
    # Удаление из Telegram (если есть)
    app = get_telegram_app()
    ...
    if app:
        try:
            await app.bot.send_message(
                MODERATION_GROUP_ID,
                f"🗑 Пользователь VK удалил публикацию:\n\n{pub['text']}\n\n❌ Удалено"
            )
        except Exception as e:
            print(f"⚠️ Не удалось уведомить модераторов: {e}", flush=True)
...
async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    ...
    print(f"🔍 VK: обработка шага, user={user_id}, state={state}, text='{text[:100]}'", flush=True)
    
    # Проверяем payload (нажатие на кнопку)
    try:
    ...
    if action == "cancel" or text.lower() in ["отменить", "отмена", "/cancel", "❌ отменить создание", "❌ отменить"]:
        clear_vk_state(user_id)
        await send_vk_message(
            user_id,
            "❌ Действие отменено.",
    ...
    if state == VKState.MAIN_MENU:
        if action == "vacancy" or 'ваканси' in text.lower():
            data = {}
            save_vk_state(user_id, VKState.V_TITLE, data)
            await send_vk_message(
                user_id,
                "📌 Название вакансии?",
                get_vk_cancel_keyboard()
            )
            return VKState.V_TITLE
        elif action == "resume" or 'резюме' in text.lower():
            data = {}
            save_vk_state(user_id, VKState.R_NAME, data)
            await send_vk_message(
                user_id,
                "👤 Ваше имя?",
                get_vk_cancel_keyboard()
    ...
    if state == VKState.V_TITLE:
        ...
        await send_vk_message(user_id, "🏢 Компания?", get_vk_cancel_keyboard())
        return VKState.V_COMPANY
    
    if state == VKState.V_COMPANY:
        data['company'] = text
        save_vk_state(user_id, VKState.V_SALARY, data)
        await send_vk_message(user_id, "💰 Зарплата?\n(напишите 'пропустить' чтобы пропустить)", get_vk_cancel_keyboard())
    ...
    if state == VKState.V_SALARY:
        ...
        await send_vk_message(user_id, "🕒 График работы?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.V_SCHEDULE:
        ...
        await send_vk_message(user_id, "📋 Описание вакансии?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.V_DESCRIPTION:
        ...
        await send_vk_message(user_id, "📞 Контакты для связи?", get_vk_cancel_keyboard())
    ...
    if state == VKState.V_CONTACT:
        ...
        await send_vk_message(
            user_id,
            f"📌 Предпросмотр вакансии:\n\n{preview}\n\nВыберите действие:",
            get_vk_preview_keyboard()
        )
    ...
    if state == VKState.V_PREVIEW:
        if action == "send" or text.upper() == 'ОТПРАВИТЬ' or text == '✅ Отправить':
            ...
            data = {}
            save_vk_state(user_id, VKState.V_TITLE, data)
            await send_vk_message(user_id, "📌 Название вакансии?", get_vk_cancel_keyboard())
            return VKState.V_TITLE
        else:
            await send_vk_message(
                user_id,
    ...
    if state == VKState.R_NAME:
        ...
        await send_vk_message(user_id, "🎂 Ваш возраст?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_AGE:
        ...
        await send_vk_message(user_id, "💼 Желаемая должность?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_POSITION:
        ...
        await send_vk_message(user_id, "📅 Опыт работы?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_EXPERIENCE:
        ...
        await send_vk_message(user_id, "🛠 Ключевые навыки?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_SKILLS:
        ...
        await send_vk_message(user_id, "🎓 Образование?\n(или 'пропустить')", get_vk_cancel_keyboard())
    ...
    if state == VKState.R_PREVIEW:
        if action == "send" or text.upper() == 'ОТПРАВИТЬ' or text == '✅ Отправить':
            ...
            await send_vk_message(
                user_id,
                "✅ Ваше резюме отправлено на модерацию!",
                get_vk_main_keyboard()
            )
            clear_vk_state(user_id)
            return VKState.MAIN_MENU
            ...
            data = {}
            save_vk_state(user_id, VKState.R_NAME, data)
            await send_vk_message(user_id, "👤 Ваше имя?", get_vk_cancel_keyboard())
            return VKState.R_NAME
        else:
            await send_vk_message(
                user_id,
    ...
    print(f"⚠️ VK: неизвестное состояние '{state}' для пользователя {user_id}, сброс", flush=True)
    clear_vk_state(user_id)
    await show_vk_main_menu(user_id)
    return VKState.MAIN_MENU
...
async def vk_listener_async():
    """Фоновый слушатель VK LongPoll"""
    global vk_api_instance, vk_longpoll
    
    print("🔵 VK: Запуск слушателя...", flush=True)
    ...
    try:
        ...
        print(f"❌ Ошибка инициализации VK: {e}", flush=True)
        traceback.print_exc()
        return
    ...
    for event in vk_longpoll.listen():
        try:
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                print(f"📨 VK: новое сообщение от user_id={user_id}, text='{text[:100]}', payload='{payload[:100]}'", flush=True)
                
                # Игнорируем сообщения от групп
                if user_id.startswith('-'):
                    continue
                
                # Если есть payload от кнопки — используем его
                if payload:
                    try:
                        ...
                        # Передаём payload как текст для обработки
                ...
                print(f"📨 VK: текущее состояние {user_id}: {state}", flush=True)
                new_state = await handle_vk_step(user_id, state, data, text)
                print(f"📨 VK: новое состояние {user_id}: {new_state}", flush=True)
...
def start_vk_listener():
    ...
    print("🔵 VK: запуск потока...", flush=True)
...
async def maintenance_scheduler():
    """Плановое обслуживание БД"""
    while True:
        try:
            await asyncio.sleep(86400)  # 24 часа
            print("🔄 Запуск планового обслуживания...", flush=True)
            archived = archive_old_publications()
            rejected = auto_reject_stale_publications()
            print(f"✅ Заархивировано: {archived}, отклонено: {rejected}", flush=True)
        except Exception as e:
...
async def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            TGState.V_TITLE: [
                make_message_handler("title", TGState.V_COMPANY, "🏢 Компания?")
            ],
            TGState.V_COMPANY: [
                make_message_handler("company", TGState.V_SALARY, "💰 Зарплата?", "v_skip_salary")
            ],
            TGState.V_SALARY: [
                make_message_handler("salary", TGState.V_SCHEDULE, "🕒 График?", "v_skip_schedule"),
                make_skip_handler(TGState.V_SCHEDULE, "🕒 График?", "v_skip_schedule")
            ],
            TGState.V_SCHEDULE: [
                make_message_handler("schedule", TGState.V_DESCRIPTION, "📋 Описание?", "v_skip_description"),
                make_skip_handler(TGState.V_DESCRIPTION, "📋 Описание?", "v_skip_description")
            ],
            TGState.V_DESCRIPTION: [
                make_message_handler("description", TGState.V_CONTACT, "📞 Контакты?"),
                make_skip_handler(TGState.V_CONTACT, "📞 Контакты?")
            ],
            TGState.V_CONTACT: [
                make_message_handler("contact", TGState.V_PREVIEW, None)
            ],
            TGState.V_PREVIEW: [
                CallbackQueryHandler(_send_publication, pattern="^v_send$"),
                CallbackQueryHandler(_cancel_publication, pattern="^v_cancel$"),
                CallbackQueryHandler(_restart_publication, pattern="^v_restart$"),
            ],
            TGState.R_NAME: [
                make_message_handler("name", TGState.R_AGE, "🎂 Возраст?", "r_skip_age")
            ],
            TGState.R_AGE: [
                make_message_handler("age", TGState.R_POSITION, "💼 Желаемая должность?", "r_skip_position"),
                make_skip_handler(TGState.R_POSITION, "💼 Желаемая должность?", "r_skip_position")
            ],
            TGState.R_POSITION: [
                make_message_handler("position", TGState.R_EXPERIENCE, "📅 Опыт работы?", "r_skip_experience"),
                make_skip_handler(TGState.R_EXPERIENCE, "📅 Опыт работы?", "r_skip_experience")
            ],
            TGState.R_EXPERIENCE: [
                make_message_handler("experience", TGState.R_SKILLS, "🛠 Навыки?", "r_skip_skills"),
                make_skip_handler(TGState.R_SKILLS, "🛠 Навыки?", "r_skip_skills")
            ],
            TGState.R_SKILLS: [
                make_message_handler("skills", TGState.R_EDUCATION, "🎓 Образование?", "r_skip_education"),
                make_skip_handler(TGState.R_EDUCATION, "🎓 Образование?", "r_skip_education")
            ],
            TGState.R_EDUCATION: [
                make_message_handler("education", TGState.R_CONTACT, "📞 Контакты?"),
                make_skip_handler(TGState.R_CONTACT, "📞 Контакты?")
            ],
            TGState.R_CONTACT: [
                make_message_handler("contact", TGState.R_PREVIEW, None)
            ],
            TGState.R_PREVIEW: [
                CallbackQueryHandler(_send_publication, pattern="^r_send$"),
                CallbackQueryHandler(_cancel_publication, pattern="^r_cancel$"),
                CallbackQueryHandler(_restart_publication, pattern="^r_restart$"),
            ],
        },
        fallbacks=[
    ...
    if VK_TOKEN and VK_GROUP_ID != 0:
        ...
        print("⚠️ VK отключен (нет VK_TOKEN или VK_GROUP_ID)", flush=True)
    ...
    print(f"📱 Telegram: активен (модерация в чате {MODERATION_GROUP_ID})", flush=True)
    print(f"📱 VK: {'активен' if (VK_TOKEN and VK_GROUP_ID != 0) else 'отключён'}", flush=True)
    print("📦 БД: SQLite", flush=True)
    print("=" * 50, flush=True)
    sys.stdout.flush()
