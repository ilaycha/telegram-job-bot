...
# ============================================
# VK: Машина состояний (ИСПРАВЛЕННАЯ)
# ============================================
async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    ...
    # Пропуск определяется только по action == "skip" или тексту
    is_skip = (action == "skip" or text.lower() in ['пропустить', '⏭ пропустить'])
    
    # ----- Функция для безопасного сохранения -----
    def save_field(field_name: str):
        """Сохранить значение поля, если это не пропуск"""
        if is_skip:
