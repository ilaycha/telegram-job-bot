...
async def _send_publication(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем только в БД (bot_data больше не используется)
...
async def moderation_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Берем данные из БД (надежнее, чем bot_data)
    pub = get_publication(pub_id)
    if not pub:
        ...
        if success:
            ...
            if pub["platform"] == "tg":
                try:
                    ...
                    print(f"⚠️ Не удалось уведомить пользователя: {e}", flush=True)
...
async def send_vk_message(user_id: str, text: str, keyboard: Optional[Dict] = None):
    ...
    try:
        ...
        if keyboard:
            ...
            print(f"📤 VK: содержимое клавиатуры: {keyboard_json}", flush=True)
        else:
...
async def handle_vk_step(user_id: str, state: str, data: dict, text: str) -> str:
    ...
    if state == VKState.MAIN_MENU:
        if 'ваканси' in text.lower() or 'vacancy' in text.lower():
            data = {}
            save_vk_state(user_id, VKState.V_TITLE, data)
            await send_vk_message(user_id, "📌 Название вакансии?")
            return VKState.V_TITLE
        elif 'резюме' in text.lower() or 'resume' in text.lower():
            data = {}
            save_vk_state(user_id, VKState.R_NAME, data)
            await send_vk_message(user_id, "👤 Ваше имя?")
    ...
    if state == VKState.V_TITLE:
        ...
        await send_vk_message(user_id, "🏢 Компания?")
    ...
    if state == VKState.V_DESCRIPTION:
        ...
        await send_vk_message(user_id, "📞 Контакты для связи?")
    ...
    if state == VKState.V_PREVIEW:
        if text.upper() == 'ОТПРАВИТЬ':
            ...
            await send_vk_message(user_id, "✅ Ваша вакансия отправлена на модерацию!")
            clear_vk_state(user_id)
            return 'completed'
        else:
            await send_vk_message(
                user_id,
                "Напишите 'ОТПРАВИТЬ' для публикации или '/cancel' для отмены."
            )
            return VKState.V_PREVIEW
    ...
    if state == VKState.R_PREVIEW:
        if text.upper() == 'ОТПРАВИТЬ':
            ...
            await send_vk_message(user_id, "✅ Ваше резюме отправлено на модерацию!")
            clear_vk_state(user_id)
            return 'completed'
        else:
            await send_vk_message(
    ...
    clear_vk_state(user_id)
    await send_vk_message(user_id, "⚠️ Что-то пошло не так. Напишите /start для начала.")
    return VKState.MAIN_MENU
    ...
    for event in vk_longpoll.listen():
        try:
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                print(f"📨 VK: новое сообщение от user_id={user_id}, text='{text[:100]}'", flush=True)
                
                # Игнорируем сообщения от групп
                if user_id.startswith('-'):
                    print(f"📨 VK: сообщение от группы {user_id}, игнорируем", flush=True)
                    continue
                
                # Команды
                if text == '/start':
                    print(f"📨 VK: команда /start от {user_id}", flush=True)
                    await show_vk_main_menu(user_id)
                    continue
                
                if text == '/cancel':
                    print(f"📨 VK: команда /cancel от {user_id}", flush=True)
                    clear_vk_state(user_id)
                    await send_vk_message(user_id, "❌ Действие отменено. Напишите /start для начала.")
                    continue
