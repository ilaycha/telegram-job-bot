...
# Отладка callback'ов
# ============================================
DEBUG_CALLBACKS = True  # Поставьте False в продакшене
...
def create_callback_data(action: str, pub_id: str) -> str:
    """Создание callback_data с проверкой длины"""
    data = f"{action}:{pub_id}"
    if len(data.encode('utf-8')) > 64:
        # Если pub_id слишком длинный, используем хеш
        import hashlib
...
# ============================================
# Telegram: Главное меню (объединенный обработчик)
# ============================================
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик главного меню - обрабатывает все callback'и меню"""
...
# ============================================
# Telegram: Обработчики команд (вне диалога)
# ============================================
...
async def handle_preview_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if DEBUG_CALLBACKS:
        print(f"🔍 PREVIEW: callback_data='{query.data}', form_type='{context.user_data.get('form_type')}'", flush=True)
...
async def main():
    ...
    print("\n" + "="*50, flush=True)
    print("DEBUG: Проверка состояний Telegram", flush=True)
    print(f"TGState.MAIN_MENU = {TGState.MAIN_MENU}", flush=True)
    print(f"TGState.V_TITLE = {TGState.V_TITLE}", flush=True)
    print(f"TGState.V_COMPANY = {TGState.V_COMPANY}", flush=True)
    print(f"TGState.V_SALARY = {TGState.V_SALARY}", flush=True)
    print(f"TGState.V_SCHEDULE = {TGState.V_SCHEDULE}", flush=True)
    print(f"TGState.V_DESCRIPTION = {TGState.V_DESCRIPTION}", flush=True)
    print(f"TGState.V_CONTACT = {TGState.V_CONTACT}", flush=True)
    print(f"TGState.V_PREVIEW = {TGState.V_PREVIEW}", flush=True)
    print(f"TGState.R_NAME = {TGState.R_NAME}", flush=True)
    print(f"TGState.R_AGE = {TGState.R_AGE}", flush=True)
    print(f"TGState.R_POSITION = {TGState.R_POSITION}", flush=True)
    print(f"TGState.R_EXPERIENCE = {TGState.R_EXPERIENCE}", flush=True)
    print(f"TGState.R_SKILLS = {TGState.R_SKILLS}", flush=True)
    print(f"TGState.R_EDUCATION = {TGState.R_EDUCATION}", flush=True)
    print(f"TGState.R_CONTACT = {TGState.R_CONTACT}", flush=True)
    print(f"TGState.R_PREVIEW = {TGState.R_PREVIEW}", flush=True)
    print("="*50 + "\n", flush=True)
    ...
    # ConversationHandler с ИСПРАВЛЕННОЙ структурой
    conv_handler = ConversationHandler(
        ...
        per_message=True  # Изменено на True для лучшей обработки
    )
    
    telegram_app.add_handler(conv_handler)
    
    # Отдельные обработчики ДО ConversationHandler
    ...
    # Команды
    telegram_app.add_handler(CommandHandler("my", my_publications_command))
    ...
    # Проверка доступа к чату модерации
    try:
