...
# Отладка callback'ов
# ============================================
DEBUG_CALLBACKS = True  # Поставьте False когда всё заработает
...
def create_callback_data(action: str, pub_id: str) -> str:
    """Создание callback_data с проверкой длины (макс 64 байта)"""
...
async def _publish_to_vk(text: str, chat_id: int):
    ...
    try:
        print(f"📤 VK: пытаюсь отправить в чат {chat_id}...", flush=True)
        result = vk_api_instance.messages.send(
            chat_id=chat_id,
            message=text,
            random_id=get_random_id()
        )
        print(f"📤 VK: результат отправки: {result}", flush=True)
        return result
    except Exception as e:
...
async def publish_to_both_platforms(pub_id: str, context) -> bool:
    ...
    print(f"📢 TG thread: {tg_thread_id}, VK chat: {vk_chat_id}", flush=True)
    ...
    # Публикация в VK
    try:
        vk_result = await _publish_to_vk(text, vk_chat_id)
        if vk_result:
            # vk_api возвращает ID сообщения как число
            ...
            print(f"✅ Опубликовано в VK (чат {vk_chat_id}): {pub_id} (msg_id={vk_message_id})", flush=True)
        else:
...
async def delete_from_both_platforms(pub_id: str, context) -> bool:
    ...
    print(f"🗑 DELETE: удаляю {pub_id}, tg_msg={pub['tg_channel_message_id']}, vk_msg={pub['vk_message_id']}, vk_chat={pub['vk_chat_id']}", flush=True)
    ...
    if pub['vk_message_id'] and pub['vk_chat_id']:
        try:
            if vk_api_instance:
                print(f"🗑 VK: удаляю сообщение {pub['vk_message_id']} из чата {pub['vk_chat_id']}", flush=True)
        ...
        print(f"⚠️ VK: нет данных для удаления (msg_id={pub['vk_message_id']}, chat={pub['vk_chat_id']})", flush=True)
...
async def main():
    ...
    print("=" * 50, flush=True)
    print("🔧 ВЕРСИЯ КОДА: 4.0 (исправлены VK chat_id и удаление)", flush=True)
    print("=" * 50, flush=True)
    ...
    print(f"📋 VK чат вакансий: {VK_CHAT_VACANCIES}", flush=True)
    print(f"📋 VK чат резюме: {VK_CHAT_RESUMES}", flush=True)
    print("=" * 50, flush=True)
    sys.stdout.flush()
