...
vk_longpoll = None
_telegram_app = None
CHANNEL_CHAT_ID = None  # <-- ДОБАВЛЕНО!
...
async def _publish_to_vk(text: str, peer_id: int):
    ...
    try:
        ...
        result = vk_api_instance.messages.send(
            peer_id=peer_id,  # Используем peer_id напрямую
            message=text,
...
async def publish_to_both_platforms(pub_id: str, context) -> bool:
    ...
    update_publication_status(
        ...
        vk_chat_id=vk_peer_id  # Сохраняем peer_id
    )
...
async def delete_from_both_platforms(pub_id: str, context) -> bool:
    ...
    # ДИАГНОСТИКА
    print(f"🔍 DELETE {pub_id}:", flush=True)
    ...
    if pub.get('tg_channel_message_id'):
        try:
            # Получаем ID канала если ещё не сохранён
            ...
            await context.bot.delete_message(
                chat_id=CHANNEL_CHAT_ID,  # Используем числовой ID
                message_id=pub['tg_channel_message_id']
            )
            print(f"✅ Удалено из Telegram: сообщение {pub['tg_channel_message_id']}", flush=True)
        except Exception as e:
            print(f"❌ Не удалось удалить из Telegram: {e}", flush=True)
            traceback.print_exc()
            success = False
    else:
    ...
    if pub.get('vk_message_id') and pub.get('vk_chat_id'):
        try:
            if vk_api_instance:
                peer_id = pub['vk_chat_id']  # Это уже peer_id
                print(f"🗑 VK: удаляю сообщение {pub['vk_message_id']} из peer_id={peer_id}", flush=True)
                
                vk_api_instance.messages.delete(
                    message_ids=[pub['vk_message_id']],
                    peer_id=peer_id,  # Обязательно передаём peer_id!
                    delete_for_all=1
                )
                print(f"✅ Удалено из VK: сообщение {pub['vk_message_id']}", flush=True)
            else:
                print(f"⚠️ VK API не инициализирован", flush=True)
                success = False
        except Exception as e:
            print(f"❌ Не удалось удалить из VK: {e}", flush=True)
            traceback.print_exc()
            success = False
    else:
        print(f"⚠️ Нет данных VK для удаления (msg_id={pub.get('vk_message_id')}, chat={pub.get('vk_chat_id')})", flush=True)
...
async def main():
    ...
    print("=" * 50, flush=True)
    print("🔧 ВЕРСИЯ КОДА: 5.0 (ИСПРАВЛЕНО УДАЛЕНИЕ)", flush=True)
    print("=" * 50, flush=True)
    ...
    # ПОЛУЧАЕМ ID КАНАЛА ДЛЯ УДАЛЕНИЯ
    try:
