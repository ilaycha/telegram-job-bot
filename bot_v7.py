...
ADMIN_ID = 468388822 #Напиши /myid боту @userinfobot, чтобы узнать свой ID
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Статистика базы данных (только для админа)"""
...
async def admin_export(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Экспорт всех публикаций в CSV (только для админа)"""
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔ Доступ запрещён.")
        return
...
# ============================================
# 8. ГЛАВНОЕ МЕНЮ (обработчик кнопок)
# ============================================
...
# ============================================
# 15. ФОНОВЫЙ ПЛАНИРОВЩИК (авто-очистка)
# ============================================
...
async def main():
    ...
    vacancy_app.add_handler(CommandHandler("my", my_publications))
    vacancy_app.add_handler(CommandHandler("adminstats", admin_stats))  # АДМИН-СТАТИСТИКА
    vacancy_app.add_handler(CommandHandler("export", admin_export))     # АДМИН-ЭКСПОРТ
